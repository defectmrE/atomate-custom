...
from monty.shutil import decompress_file, compress_file
...
from pymatgen.io.vasp import BSVasprun, Vasprun, Outcar, Locpot, Chgcar, \
    parse_defect_states, parse_qis_data, Procar, Wavecar, Eigenval
...
class VaspDrone(AbstractDrone):
    ...
    def __init__(self, runs=None, parse_dos="auto", bandstructure_mode="auto",
                 ...
                 parse_eigenval=False, defect_wf_parsing=None, qis_data_dict={}):
        """
        ...
        self.parse_eigenval = parse_eigenval
        self.defect_wf_parsing = defect_wf_parsing
        self.qis_data_dict = qis_data_dict
    ...
    def assimilate(self, path):
        ...
        eigenval_files = self.filter_files(path, file_pattern="EIGENVAL")
        if len(vasprun_files) > 0 and len(outcar_files) > 0 and len(eigenval_files) > 0:
            d = self.generate_doc(path, vasprun_files, outcar_files, eigenval_files)
    ...
    def generate_doc(self, dir_name, vasprun_files, outcar_files, eigenval_files):
        """
            ...
            if self.defect_wf_parsing:
                ...
                for i, d_calc in enumerate(d["calcs_reversed"]):
                    if d_calc.get("output"):
                        ...
                        filename = list(eigenval_files.values())[i]
                        eigenval_file = os.path.join(dir_name, filename)
                        egv = Eigenval(eigenval_file)
                        eigenvalues = egv.eigenvalues.copy()
                        kpoint_weights = egv.kpoints_weights
                        egv_eigenvalue_dict = {'eigenvalues': eigenvalues, 'kpoint_weights': kpoint_weights}
                        d_calc["output"].update({"egv_eigenvalue_dict": egv_eigenvalue_dict})

                        if len(procar_paths) and len(wavecar_paths):
                            procar = Procar(procar_paths[i])

                            gam = self.qis_data_dict.get("gam", False)
                            ref_ind = self.qis_data_dict.get("ref_ind", 3.88)
                            vbm = self.qis_data_dict.get("vbm", 5.6158)
                            cbm = self.qis_data_dict.get("cbm", 6.2268)

                            # TODO: might need to add self.ref_ind, self.gam, self.vbm, self.cbm
                            defect_data = parse_qis_data(wavecar_paths[i], procar, eigenvalues, gam=gam,
                                                         ref_ind=ref_ind, vbm=vbm, cbm=cbm
                                                         )

                            d_calc['output'].update({"qis_data": defect_data})
                            compress_file(procar_paths[i])
                            compress_file(wavecar_paths[i])

            if self.parse_eigenval:
                for i, d_calc in enumerate(d['calcs_reversed']):
                    filename = list(eigenval_files.values())[i]
                    eigenval_file = os.path.join(dir_name, filename)
                    egv = Eigenval(eigenval_file)
